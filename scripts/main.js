...
Hooks.once("init", () => {
    // Registra uma ficha customizada com nova exibição das perícias
    class CustomCharacterSheet extends dnd5e.applications.actor.ActorSheet5eCharacter {
      static get defaultOptions() {
        const options = super.defaultOptions;
        options.classes = ["dnd5e", "sheet", "actor", "character", "my-custom-sheet"];
        options.width = 800;
        options.height = 700;
        options.template = "modules/my-skill-system/templates/custom-sheet.html";
        return options;
      }
  
      getData() {
        const context = super.getData();
        const habilidades = context.actor.system.abilities;
        const skills = CONFIG.DND5E.skills;
        const flags = context.actor.flags["my-skill-system"]?.skills || {};
  
        context.customSkills = Object.entries(skills).map(([key, meta]) => {
          const pontos = flags[key] || 0;
          const mod = meta.ability ? habilidades[meta.ability].mod : 0;
          const prof = context.actor.system.attributes.prof || 0;
          const total = pontos > 0 ? pontos + mod + prof : mod;
          return {
            key,
            label: meta.label,
    ...
    Actors.registerSheet("my-skill-system", CustomCharacterSheet, {
      types: ["character"],
      makeDefault: true
    });
